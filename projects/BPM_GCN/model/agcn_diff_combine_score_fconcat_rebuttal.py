...
class Model(nn.Module):
    def __init__(self, num_class=4, num_point=16, num_person=1, graph=None, graph_args=dict(), in_channels_p=3,in_channels_m=8):
        ...
        self.fc_classifier_combine = nn.Linear(512, num_class)
        ...
        nn.init.normal_(self.fc_classifier_combine.weight, 0, math.sqrt(2. / num_class))
    ...
    def forward(self, x_p,x_m):
        ...
        return self.fc1_classifier_p(x_p),self.fc2_aff(x_p),self.fc1_classifier_m(x_m),self.fc_classifier_combine(torch.cat((x_m,x_p),1))
