...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None,use_nl=False):
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], shortcut_type, stride=2, nonlocal_mod=nonlocal_mod)
        self.layer3 = self._make_layer(block, 256, layers[2], shortcut_type, stride=2, nonlocal_mod=nonlocal_mod)
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1, nonlocal_mod=1000):
        downsample = None
        # downsample case
        ...
        for i in range(1, blocks):
            layers.append(block(self.in_planes, planes,use_nl=(i%nonlocal_mod==nonlocal_mod-1)))
...
def resnet101(n_classes, sample_duration, sample_size):
    ...
    model = ResNet(block=Bottleneck, layers=[3, 4, 23, 3], shortcut_type='B', num_classes=n_classes,
                   sample_duration=sample_duration, sample_size=sample_size,use_nl=True)
    return model
...
def pretrained_resnet101(snippet_duration: int,
                         ...
                         n_classes=8,
                         ft_begin_index=5,
                         pretrained_resnet101_path="../pretrained/resnet-101-kinetics.pth"):
...
def get_fine_tuning_parameters(model, ft_begin_index):
    ...
    for k, v in model.named_parameters():
        for ft_module in ft_module_names:  # fc
            if ft_module in k or 'nl' in k:#here add nonlocal to update
...
if __name__=="__main__":
    model,parameters=pretrained_resnet101(snippet_duration=16,sample_size=112)
    a=torch.zeros((1,3,16,112,112)).cuda()
    #print(model(a).shape)