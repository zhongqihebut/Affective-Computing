...
original_execute = Pool3d.execute

# 定义修改后的 execute 方法
def modified_execute(self, x):
    N, C, D, H, W = x.shape
    ...
    # 调用原始的 execute 方法（如果需要）
    return original_execute(self, x)

# 使用猴子补丁替换原始方法
Pool3d.execute = modified_execute
...
def pretrained_resnet101(snippet_duration: int,
    ...
    pretrain = jt.load(pretrained_resnet101_path)
    # ---------------------------------------------------------------- #
